...
const Programacion = () => {
  ...
  const handleSaveCell = ({ cell, row, value }) => {
    ...
    const updateRow = (rows) => {
      return rows.map((item) => {
        if (item.id === row.original.id) {
          return { ...item, [cell.column.id]: value };
    ...
    const updated = updateRow(data);
    setData(updated);
  };

  const handleAddActivity = (parentId = null) => {
    lastId += 1;
  ...
  const table = useMaterialReactTable({
    ...
    defaultColumn: {
      muiTableBodyEditTextFieldProps: () => ({
        autoFocus: true,
      }),
      muiTableBodyCellProps: ({ cell }) => ({
        onKeyDown: (event) => {
          const rowIdActual = cell.row.original.id;
          // const parentIdActual = cell.row.original.parentId ?? 0;
          ...
          if (event.key === "s" || event.key === "S") {
            ...
            flatData.push({
              id: ++lastId,
              parentId: rowIdActual, // subactividad del actual
              nombre: "Nueva Subactividad",
              fechaInicio: "",
              fechaFin: "",
              responsable: "",
              predecesorId: null,
          ...
          if (event.key === "F2") {
            ...
            setTimeout(() => {
              ...
              if (input) {
                input.focus();
                input.select(); // para seleccionar el contenido al estilo Excel
              }
            }, 0);
          }
          if (event.key === "Delete") {
  ...
  return (
    <div className="p-4">
      <h2 className="text-2xl font-bold mb-4">
        Programación WBS (SIGET - PRO v4)
      </h2>

      <Button
        variant="contained"
        color="primary"
        onClick={() => handleAddActivity(null)}
        sx={{ mb: 2 }}
      >
