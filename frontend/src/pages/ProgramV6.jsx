    ...
    defaultColumn: {
      ...
      muiTableBodyCellProps: ({ cell }) => ({
        onKeyDown: (event) => {
          ...
          const parentId = cell.row.original.parentId ?? 0;

          if (event.key === "A" || event.key === "a") {
            const newParentId = event.shiftKey ? 0 : parentId;
            const flatData = flattenTree(data);
            flatData.push({
              id: ++lastId,
          ...
          if (event.key === "Delete") {
            ...
            const rowId = cell.row.original.id; // <-- este es el ID real de tu dato

            setData((prev) => {
              const updateRow = (rows) =>
                rows.map((item) => {
                  if (item.id === rowId) {
                    // Clean only the current cell (column)
